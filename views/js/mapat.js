...
navigator.geolocation.getCurrentPosition(
    (pos) => {
        const {coords} = pos
        console.log(coords)
        L.marker([coords.latitude, coords.longitude], {icon: myIcon}).addTo(myMap)
    },
    (err) => {
        console.log(err)
    },
...
var myIcon01 = L.icon({
    iconUrl: './imagens/iazul.png',
    iconSize: [30,30],

});
//função para pegar a posição do usuário e marcar com o círculo

function display(){
    if(!navigator.geolocation){
        console.log('geolocation não é suportado nesse navegador!')
    }else{
        navigator.geolocation.getCurrentPosition(position => {
            ...
            let marker = L.marker([data.lat, data.long], {icon: myIcon01}).addTo(myMap)
...
document.getElementById('user-location').addEventListener('click', function(){
   // new L.map('myMap').setView([position.coords.latitude, position.coords.longitude], 13).addTo(myMap);
